...
std::shared_ptr<Statement> Parser::parseDeclaration(bool isConst) {
  ...
  auto statement = std::make_shared<DeclarationStatement>(DeclarationStatement{
      makeDebugInfo(tBegin, tEnd), isConst, std::move(type), name, expression});
...
std::shared_ptr<Expression> Parser::exponentiation() {
  ...
  case Token::Kind::FloatLiteral: {
    ...
    scan();
    return std::make_shared<ConstantFP>(ConstantFP{val});
  }
  case Token::Kind::IntegerLiteral: {
    auto val = current().val;
    scan();
    return std::make_shared<ConstantInt>(ConstantInt{val});
  }
...
std::pair<std::unique_ptr<Type>, bool> Parser::parseType() {
  std::unique_ptr<Type> type;
  bool isOldStyleDeclaration = false;

  switch (current().kind) {
  case Token::Kind::CReg:
    type = std::make_unique<BitType>(BitType());
    isOldStyleDeclaration = true;
    break;
  case Token::Kind::Qreg:
    type = std::make_unique<QubitType>(QubitType());
    isOldStyleDeclaration = true;
    break;
  case Token::Kind::Int:
    type = std::make_unique<IntType>(IntType());
    break;
  case Token::Kind::Uint:
    type = std::make_unique<UintType>(UintType());
    break;
  case Token::Kind::Bit:
    type = std::make_unique<BitType>(BitType());
    break;
  ...
  case Token::Kind::Float:
    type = std::make_unique<FloatType>(FloatType());
    break;
  case Token::Kind::Angle:
    type = std::make_unique<AngleType>(AngleType());
    break;
  case Token::Kind::Bool:
    type = std::make_unique<BoolType>(BoolType());
    break;
  case Token::Kind::Duration:
    type = std::make_unique<DurationType>(DurationType());
    break;
  default:
