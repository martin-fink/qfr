...
class OpenQasm3Parser: public InstVisitor {
    ...
    std::map<icu::UnicodeString, std::shared_ptr<Gate>> gates = {
            {"gphase", std::make_shared<StandardGate>(StandardGate({0, 0, 1, qc::GPhase}))},
            {"U", std::make_shared<StandardGate>(StandardGate({0, 1, 3, qc::U3}))},
            //            {"i", std::make_shared<StandardGate>(StandardGate({0, 1, 0, qc::I}))},
            ...
            //            {"sdg", std::make_shared<StandardGate>(StandardGate({0, 1, 0, qc::Sdag}))},
            //            {"t", std::make_shared<StandardGate>(StandardGate({0, 1, 0, qc::T}))},
            //            {"tdg", std::make_shared<StandardGate>(StandardGate({0, 1, 0, qc::Tdag}))},
            //            {"sx", std::make_shared<StandardGate>(StandardGate({0, 1, 0, qc::SX}))},
            //            {"sxdg", std::make_shared<StandardGate>(StandardGate({0, 1, 0, qc::SXdag}))},
            ...
            //            {"u1", std::make_shared<StandardGate>(StandardGate({0, 1, 1, qc::Phase}))},
            //            {"p", std::make_shared<StandardGate>(StandardGate({0, 1, 1, qc::Phase}))},
            //            {"u2", std::make_shared<StandardGate>(StandardGate({0, 1, 2, qc::U2}))},
            //            {"u3", std::make_shared<StandardGate>(StandardGate({0, 1, 3, qc::U3}))},
            //            {"u", std::make_shared<StandardGate>(StandardGate({0, 1, 3, qc::U3}))},
            //            {"teleport", std::make_shared<StandardGate>(StandardGate({0, 3, 0, qc::Teleportation}))},
            //            {"swap", std::make_shared<StandardGate>(StandardGate({0, 2, 0, qc::SWAP}))},
            //            {"iswap", std::make_shared<StandardGate>(StandardGate({0, 2, 0, qc::iSWAP}))},
            //            {"cnot", std::make_shared<StandardGate>(StandardGate({1, 1, 0, qc::X}))},
            ...
            //            {"xx_minus_yy", std::make_shared<StandardGate>(StandardGate({0, 2, 2, qc::XXminusYY}))},
            //            {"xx_plus_yy", std::make_shared<StandardGate>(StandardGate({0, 2, 2, qc::XXplusYY}))},
    };

    void error(const icu::UnicodeString& message, const std::shared_ptr<DebugInfo>& debugInfo) {
    ...
    void initializeBuiltins() {
        auto addConstantDeclaration = [this](const icu::UnicodeString& identifier, Declaration::Type type, uint8_t width, std::shared_ptr<Expression> expr) {
            auto decl = Declaration{identifier, std::move(expr), type, width, true};
            declarations->emplace(identifier, decl);
            return decl;
        };

        addConstantDeclaration("pi", Declaration::Type::Float, 1, std::make_shared<ConstantFP>(ConstantFP(qc::PI)));
        addConstantDeclaration("π", Declaration::Type::Float, 1, std::make_shared<ConstantFP>(ConstantFP(qc::PI)));
    }

    bool translateGateOperand(const std::shared_ptr<GateOperand>& gateOperand, std::vector<qc::Qubit>& qubits, const qc::QuantumRegisterMap& qregs, std::shared_ptr<DebugInfo> debugInfo) {
        return translateGateOperand(gateOperand->identifier, gateOperand->expression, qubits, qregs, debugInfo);
    }

    bool translateGateOperand(const icu::UnicodeString& gateIdentifier, const std::shared_ptr<Expression>& indexExpr, std::vector<qc::Qubit>& qubits, const qc::QuantumRegisterMap& qregs, const std::shared_ptr<DebugInfo>& debugInfo) {
        ...
        if (indexExpr != nullptr) {
            ...
            try {
                auto result = std::any_cast<std::shared_ptr<Constant>>(indexExpr->accept(&visitor));
                ...
                if (static_cast<uint64_t>(result->getInt()) > qubit.second) {
                    error("Index expression must be smaller than the width of the quantum register.", debugInfo);
                    return false;
                }
    ...
    bool translateBitOperand(const icu::UnicodeString& bitIdentifier, const std::shared_ptr<Expression>& indexExpr, std::vector<qc::Bit>& bits, const std::shared_ptr<DebugInfo>& debugInfo) {
        std::string identifier{};
        ...
        if (indexExpr != nullptr) {
            ...
            try {
                ...
                if (static_cast<uint64_t>(result->getInt()) > creg.second) {
                    error("Index expression must be smaller than the width of the classical register.", debugInfo);
                    return false;
                }
...
public:
    ...
    bool visitProgram(const std::vector<std::shared_ptr<Statement>>& program) {
        ...
        // To do this, we need to insert make sure that erroneous declarations actually insert a dummy entry;
        // also, we need to synchronize to the next semicolon, to make sure we don't do some weird stuff and report
        // false errors.
        return std::all_of(program.begin(), program.end(), [this](const auto& statement) {
            statement->accept(this);
            return !hasError;
        });
    }

    void visitVersionDeclaration(std::shared_ptr<VersionDeclaration> /*versionDeclaration*/) override {
        // TODO: do something with the version declaration
    }

    void visitDeclarationStatement(std::shared_ptr<DeclarationStatement> declarationStatement) override {
        ...
        if (auto designatorExpr = declarationStatement->type->getDesignator()) {
            ConstExpressionPropagation visitor(declarations);
            try {
                auto result = std::any_cast<std::shared_ptr<Constant>>(designatorExpr->accept(&visitor));

                if (result == nullptr) {
                    error("Designator expression must be a constant integer.", declarationStatement->debugInfo);
                    return;
                }
                if (!result->isInt()) {
                    error("Designator expression must be a constant integer.", declarationStatement->debugInfo);
                ...
                if (result->getInt() <= 0) {
                    error("Designator expression must be a positive integer > 0.", declarationStatement->debugInfo);
                    return;
                }
        ...
        private:
            ...
            void insertDeclaration(Declaration::Type type) {
                ...
                env->emplace(unicodeIdentifier, Declaration{unicodeIdentifier, expr, type, designator, declarationStatement->isConst});
            }

        public:
            explicit TyVisitor(OpenQasm3Parser* parser, qc::QuantumComputation* qc, const icu::UnicodeString& identifier, uint8_t designator, std::shared_ptr<Environment> env, std::shared_ptr<DeclarationStatement> declarationStatement):
                parser(parser), qc(qc), unicodeIdentifier(identifier), designator(designator), env(std::move(env)), declarationStatement(std::move(declarationStatement)) {
            ...
            void visitAngleType(AngleType* /*type*/) override {
                parser->error("Angle type not supported yet.", declarationStatement->debugInfo);
            }
            ...
            void visitDurationType(DurationType* /*type*/) override {
                parser->error("Duration type not supported yet.", declarationStatement->debugInfo);
            }
            void visitStretchType(StretchType* /*type*/) override {
                parser->error("Stretch type not supported yet.", declarationStatement->debugInfo);
            }
            void visitComplexType(ComplexType* /*type*/) override {
                parser->error("Complex type not supported yet.", declarationStatement->debugInfo);
            }
            void visitQubitType(QubitType* /*type*/) override {
                if (declarationStatement->isConst) {
                    parser->error("Cannot declare a constant qubit register.", declarationStatement->debugInfo);
                    return;
                }
        ...
        TyVisitor tyVisitor(this, qc, identifier, designator, declarations, declarationStatement);
        declarationStatement->type->accept(&tyVisitor);

        // for now, only support measure initialization and constant initialization
        if (declarationStatement->isConst) {
            ConstExpressionPropagation visitor(declarations);
            auto                       result = std::any_cast<std::shared_ptr<Constant>>(declarationStatement->expression->accept(&visitor));
            if (result == nullptr) {
                error("Const variable initialized with non-const value.", declarationStatement->debugInfo);
                return;
            }
            ...
            if (auto measureExpression = std::dynamic_pointer_cast<MeasureExpression>(declarationStatement->expression->expression)) {
                if (declarationStatement->isConst) {
                    error("Cannot initialize a const register with a measure statement.", declarationStatement->debugInfo);
                    return;
                }
                visitMeasureAssignment(declarations->at(identifier), nullptr, measureExpression, declarationStatement->debugInfo);
            }
        }
    }

    void visitAssignmentStatement(std::shared_ptr<AssignmentStatement> assignmentStatement) override {
        ...
        if (auto measureExpression = std::dynamic_pointer_cast<MeasureExpression>(assignmentStatement->expression->expression)) {
            visitMeasureAssignment(declaration->second, assignmentStatement->indexExpression, measureExpression, assignmentStatement->debugInfo);
            return;
        }

        // In the future, handle classical computation.
        error("Classical computation not supported.", assignmentStatement->debugInfo);
    }

    void visitInitialLayout(std::shared_ptr<InitialLayout> initialLayout) override {
        if (!qc->initialLayout.empty()) {
            error("Multiple initial layout specifications found.", initialLayout->debugInfo);
            return;
        }
        qc->initialLayout = initialLayout->permutation;
    }

    void visitOutputPermutation(std::shared_ptr<OutputPermutation> outputPermutation) override {
        if (!qc->outputPermutation.empty()) {
            error("Multiple output permutation specifications found.", outputPermutation->debugInfo);
            return;
        }
        qc->outputPermutation = outputPermutation->permutation;
    }

    void visitGateStatement(std::shared_ptr<GateDeclaration> gateStatement) override {
        ...
        auto compoundGate = std::make_shared<CompoundGate>(CompoundGate(parameterIdentifiers, qubitIdentifiers, gateStatement->statements, std::move(globalEnv)));

        gates.emplace(identifier, compoundGate);
    }

    void visitGateCallStatement(std::shared_ptr<GateCallStatement> gateCallStatement) override {
        ...
        auto op = evaluateGateCall(gateCallStatement, gateCallStatement->identifier, gateCallStatement->arguments, gateCallStatement->operands, declarations, qregs);
        if (op != nullptr) {
            qc->emplace_back(std::move(op));
        }
    }

    std::unique_ptr<qc::Operation> evaluateGateCall(const std::shared_ptr<GateCallStatement>& gateCallStatement, const icu::UnicodeString& identifier,
                                                    const std::vector<std::shared_ptr<Expression>>& parameters, std::vector<std::shared_ptr<GateOperand>> targets,
                                                    const std::shared_ptr<Environment>& env, qc::QuantumRegisterMap& qregs, bool invertOperation = false) {
        ...
        // since standard gates may define a number of control targets, we first need to handle those
        ...
        for (const auto& modifier: gateCallStatement->modifiers) {
            if (auto* ctrlModifier = dynamic_cast<CtrlGateModifier*>(modifier.get()); ctrlModifier != nullptr) {
                size_t n{0};
                if (ctrlModifier->expression == nullptr) {
                    n = 1;
                } else {
                    ConstExpressionPropagation visitor{env};
                    auto                       result = std::any_cast<std::shared_ptr<Constant>>(ctrlModifier->expression->accept(&visitor));
                    if (result == nullptr) {
                        error("Only const expressions are supported.", gateCallStatement->debugInfo);
                        return nullptr;
                    }
                    if (!result->isInt() || result->getInt() <= 0) {
                        error("Only positive integer values are supported.", gateCallStatement->debugInfo);
                        return nullptr;
                    }
                ...
                nControls += n;
            } else if (auto *invModifier = dynamic_cast<InvGateModifier*>(modifier.get()); invModifier != nullptr) {
                // if we have an even number of inv modifiers, they cancel each other out
                invertOperation = !invertOperation;
            } else {
                error("Only ctrl/negctrl/inv modifiers are supported.", gateCallStatement->debugInfo);
                return nullptr;
            }
        }
        targets.erase(targets.begin(), targets.begin() + static_cast<int64_t>(nControls));
        ...
        for (const auto& param: parameters) {
            auto result = std::any_cast<std::shared_ptr<Constant>>(param->accept(&visitor));
        ...
        for (const auto& target: targets) {
            qc::Targets t{};
            if (!translateGateOperand(target, t, qregs, gateCallStatement->debugInfo)) {
                return nullptr;
            }
            ...
            if (t.size() > 1) {
                if (broadcastingWidth != 1 && t.size() != broadcastingWidth) {
                    error("When broadcasting, all registers must be of the same width.", gateCallStatement->debugInfo);
                    return nullptr;
                }
        ...
        for (const auto& [control, type]: controls) {
            qc::Targets c{};
            if (!translateGateOperand(control, c, qregs, gateCallStatement->debugInfo)) {
                return nullptr;
            }

            controlBits.emplace_back(qc::Control{c[0], type ? qc::Control::Type::Pos : qc::Control::Type::Neg});
        ...
        auto applyGateOperation = [this, &gate, &evaluatedParameters, &env, &gateCallStatement, &invertOperation](qc::Targets targetBits, std::vector<qc::Control> controlBits) -> std::unique_ptr<qc::Operation> {
            if (auto* standardGate = dynamic_cast<StandardGate*>(gate.get())) {
                ...
                return std::make_unique<qc::StandardOperation>(qc->getNqubits(), qc::Controls{controlBits.begin(), controlBits.end()}, targetBits, standardGate->info.type, params);
            }
            if (auto* compoundGate = dynamic_cast<CompoundGate*>(gate.get())) {
                ...
                for (const auto& nestedGate: compoundGate->body) {
                    for (const auto& operand: nestedGate->operands) {
                        ...
                        if (operand->expression != nullptr && std::find(compoundGate->targetNames.begin(), compoundGate->targetNames.end(), operand->identifier) != compoundGate->targetNames.end()) {
                            error("Gate arguments cannot be indexed within gate body.", gateCallStatement->debugInfo);
                            return nullptr;
                        }
                    }

                    auto nestedOp = evaluateGateCall(nestedGate, nestedGate->identifier, nestedGate->arguments, nestedGate->operands, nestedEnv, nestedQubits, invertOperation);
                    if (nestedOp == nullptr) {
                        return nullptr;
                    }
    ...
    void visitMeasureAssignment(Declaration& classicalReg, const std::shared_ptr<Expression>& indexExpression,
                                const std::shared_ptr<MeasureExpression>& measureExpression, const std::shared_ptr<DebugInfo>& debugInfo) {
        if (classicalReg.type != Declaration::Type::Bit) {
            error("Measure expression can only be assigned to a bit register.", debugInfo);
            return;
        }

        std::vector<qc::Qubit> qubits{};
        std::vector<qc::Bit>   bits{};
        if (!translateGateOperand(measureExpression->gate, qubits, qc->getQregs(), debugInfo) ||
            !translateBitOperand(classicalReg.identifier, indexExpression, bits, debugInfo)) {
            return;
        }

        if (qubits.size() != bits.size()) {
            error("Classical and quantum register must have the same width in measure statement.", debugInfo);
            return;
        }

        auto op = std::make_unique<qc::NonUnitaryOperation>(qc->getNqubits(), qubits, bits);
        qc->emplace_back(std::move(op));
    }
    ...
    void pushDownControls(qc::CompoundOperation* op) {
        ...
        for (auto& nestedOp: op->getOps()) {
            nestedOp->getControls().insert(op->getControls().begin(), op->getControls().end());
            newOps.push_back(legalizeControls(std::move(nestedOp)));
        }

        op->getOps() = std::move(newOps);

        // now that all controls have been pushed down, the compound operation has no more controls
        op->setControls(qc::Controls{});
    }

    std::unique_ptr<qc::Operation> legalizeControls(std::unique_ptr<qc::Operation> op) {
        if (op->getControls().empty()) {
            return op;
        }

        if (auto compoundOp = dynamic_unique_cast<qc::CompoundOperation>(std::move(op))) {
            pushDownControls(compoundOp.get());
            return compoundOp;
        }
        if (auto standardOp = dynamic_unique_cast<qc::StandardOperation>(std::move(op))) {
            if (standardOp->getType() == qc::GPhase) {
                // a controlled `gphase a` is replaced with a single-qubit U(0, 0, a) gate
                ...
                for (auto control: standardOp->getControls()) {
                    auto newStdOp = std::make_unique<qc::StandardOperation>(qc->getNqubits(), qc::Controls{}, qc::Targets{control.qubit}, qc::U3, std::vector<qc::fp>{0, 0, angle});
    ...
    void visitBarrierStatement(std::shared_ptr<BarrierStatement> barrierStatement) override {
        std::vector<qc::Qubit> qubits{};
        for (const auto& gate: barrierStatement->gates) {
            translateGateOperand(gate, qubits, qc->getQregs(), barrierStatement->debugInfo);
        }

        auto op = std::make_unique<qc::NonUnitaryOperation>(qc->getNqubits(), qubits, qc::Barrier);
        qc->emplace_back(std::move(op));
    }
    void visitResetStatement(std::shared_ptr<ResetStatement> resetStatement) override {
        std::vector<qc::Qubit> qubits{};
        translateGateOperand(resetStatement->gate, qubits, qc->getQregs(), resetStatement->debugInfo);
        auto op = std::make_unique<qc::NonUnitaryOperation>(qc->getNqubits(), qubits, qc::Reset);
        qc->emplace_back(std::move(op));
    }
};
